...
#note:删除可以直接dststr=""
print("replace:format:str.replace(srcstr,dststr,start,end)=" +
      str4.replace("cat", "doge"))
print("raw str=" + str4)
#split的常用方法,取文件后缀
...
for t_file in file:
    if t_file.split(".")[-1] == "py":
        print(t_file)
        print(t_file.strip())  #strip可以把空格消除
...
def Get_subseq():
    return "".join(random.sample(
        data, 4))  #random.sample(data,4):随机取出4个不一样的值,生成一个list
